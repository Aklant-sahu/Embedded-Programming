void autopickup()
{
  ...
  if((dist1-dist2>minerror || dist2-dist1>minerror) && (dist1>6 || dist2>6))
  {
rectify();
  }
  else{
...
void rectify(){
  ...
  digitalWrite(dirL2, LOW);
  digitalWrite(dirR2, HIGH);
    analogWrite(pwmL1,autospeed-40);
  analogWrite(pwmR1,autospeed-40);
  analogWrite(pwmL2,autospeed-40);
  analogWrite(pwmR2,autospeed-40);

    Serial.println("CLockwise Slow3");
  }
      else if(dist2-dist1>=minerror3){
